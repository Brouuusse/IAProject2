...
from pacman_module.game import manhattanDistance, PriorityQueue
...
class BeliefStateAgent(Agent):
    ...
    def transition_matrix(self, walls, position):
        ...
        position_indices = {
            tuple(pos): idx for idx, pos in enumerate(
                valid_positions
                )
            }
        ...
        for (
            current_x, current_y
        ), current_idx in position_indices.items():
            neighbor_indices = np.where(
                distance_matrix[current_idx] == 1
                )[0]
            neighbors = valid_positions[neighbor_indices]

            distances_to_pacman = np.array(
                [
                    manhattanDistance(
            ...
            current_distance_to_pacman = manhattanDistance(
                (current_x, current_y), position
                )

            for neighbor, neighbor_distance_to_pacman in zip(
                neighbors, distances_to_pacman
            ):
                neighbor_x, neighbor_y = neighbor
                if (
                    neighbor_distance_to_pacman < current_distance_to_pacman
                ):
                    transition_matrix[
            ...
            transition_matrix[
                current_x, current_y
                    ...
                    transition_matrix[current_x, current_y]
                    )
                ...
                if abs(difference) <= 2:
                    ...
                    observation_matrix[
                        x
                            ...
                            y
                            ] = BeliefStateAgent.probability_distribution[
                        probability_index
                        ]
    ...
    def update(self, walls, belief, evidence, position):
        ...
        for current_x, current_y in valid_positions:
            transition_probability_sum = np.sum(
                transition_matrix[:, :, current_x, current_y] * belief
                )
            updated_belief[
                current_x, current_y
                ] = observation_matrix[
...
class PacmanAgent(Agent):
    ...
    def _get_action(self, walls, beliefs, eaten, position):
        ...
        if self.has_moved and self.target == -1 or eaten[self.target]:
            best_position, self.target = self._get_best_target_position(
                walls, beliefs, eaten, position
                )
        else:
            best_position = self._get_best_position_for_target(
                walls, beliefs, position
                )
    ...
    def _get_best_target_position(self, walls, beliefs, eaten, position):
        ...
        candidates = [
            (
                self._get_candidate_position_for_ghost(
                    walls, beliefs, ghost_index
                    ), ghost_index
                    )
    ...
    def _find_best_action_to_target(self, walls, position, best_position):
        ...
        priority_queue.push(
            (position, [], 0), manhattanDistance(
                position, best_position
                )
                )

        visited_positions = set()
        direction_priorities = self._get_direction_priorities(
            position, best_position
            )

        while not priority_queue.isEmpty():
            _, (
                (current_x, current_y), current_path, cost
                ) = priority_queue.pop()
    ...
    def _expand_priority_queue(
        ...
        for _, action in direction_priorities:
            ...
            if (
                0 <= next_x < walls.width
                ) and (
                    0 <= next_y < walls.height
                    ) and not (
                        walls[next_x][next_y]
                        ):
                new_cost = cost + 1
                priority = new_cost + manhattanDistance(
                    (
                        next_x, next_y
                        ), best_position
                        )
